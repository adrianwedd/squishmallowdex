...
| 5 | 2026-01-04 | Add work distribution: Codex (1,2,4.5) → Claude (3,4,5), sequential commits |
...
| Write CSV | Yes | **No** |
| Write HTML | Yes | **No** |
| Console output | Yes | Yes (dry-run banner + per-URL "[DRY RUN]" lines) |
...
```
  ...
  ℹ️  [DRY RUN] Would fetch: Fifi the Fox
  ℹ️  [DRY RUN] Would fetch: Gordon the Shark
  ℹ️  [DRY RUN] Would fetch: Wendy the Frog
  ℹ️  [DRY RUN] Would save batch 1
  ...
    SQUISHMALLOWDEX DRY RUN COMPLETE!
    Would collect:        50
    Already have:         0
```
...
# Update CSS generation to use responsive sizing:
def _generate_css(thumb_size: int = 100) -> str:
    thumb_size_768 = int(thumb_size * 0.78)
    thumb_size_480 = int(thumb_size * 0.67)
...
| --thumb-size | Actual Size | Display Size (desktop / tablet / mobile) | File Impact |
|--------------|-------------|------------------------------------------|-------------|
| 50 | 50px | 50 / 39 / 33px | ~1KB/image |
| 100 (default) | 100px | 100 / 78 / 67px | ~2.5KB/image |
| 150 | 150px | 150 / 117 / 100px | ~5KB/image |
| 200 | 200px | 200 / 156 / 134px | ~8KB/image |

**Rationale:** Display size matches the actual thumbnail size on desktop, with proportional downscaling on smaller screens to keep the layout readable.
...
```
squishmallowdex.py
├── Imports (reorder per PEP 8)
├── Constants (new section: MAX_BIO_LENGTH, THUMBNAIL_SIZE, etc.)
├── PIL_RESAMPLING (Pillow 10+ compatibility)
├── AdventureLog._ANSI_PATTERN (precompiled regex)
├── _get_cached_image_path() (new helper)
├── _sync_progress_file() (new helper)
├── image_to_base64_thumbnail (error logging)
├── render_html
│   ├── HTML_COLUMNS constant
│   ├── escape() all text fields
│   ├── Dynamic CSS for thumb_size
│   └── (Optional) Extract to helper functions
├── build_html_rows (cache-aware embedding)
├── main
│   ├── try/finally for session/log cleanup
│   ├── --dry-run flag and logic
│   ├── --thumb-size flag
│   ├── --json flag
│   └── Batch-level progress sync
└── ~200 lines changed total

test_squishmallowdex.py
├── TestXSSPrevention (new)
│   ├── test_xss_in_name
│   ├── test_xss_in_bio
│   └── test_xss_in_type
├── TestImageToBase64Thumbnail (new)
│   ├── test_local_file
│   ├── test_url_fetch
│   └── test_corrupt_image
├── TestDryRun (new)
├── TestResourceCleanup (new)
└── TestNetworkMocking (new)
└── ~200 lines added

README.md
├── Commands section (new examples)
└── ~20 lines added
...
├── --json documentation
├── --thumb-size documentation
├── Running Tests section (new)
└── ~50 lines added
